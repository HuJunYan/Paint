...
public class Utils {
    ...
    public static String getUUID(Context context) {
        String uuid = UserUtil.getUUID(context);
        if (!"".equals(uuid)) {
            return uuid;
        }
        try {
            ...
            UserUtil.setUUID(context, uniqueId);
            return uniqueId;
        } catch (Exception e) {
            e.printStackTrace();
            String round = System.currentTimeMillis() + "-" + new Random().nextInt(99999) + 10000 + "-" + new Random().nextInt(99999) + 10000;
            UserUtil.setUUID(context, round);
            return round;
        }
    }
    ...
    // 传入一个手机号判断是否是合法的手机号码
    public static Boolean isPhoneNumber(Context context, String number, Button button) {
        if (TextUtils.isEmpty(number)) {
//				ToastUtil.showToast(context, R.string.phonenumbercannotnull);
            button.setClickable(true);
            return false;
        } else if (number.length() != 11) {
//				ToastUtil.showToast(context, R.string.Please_enter_correct_phonenumber);
            button.setClickable(true);
            return false;
